...
impl Chain {
    ...
    pub fn print_specific_block(&self, index: usize) {
        println!("{:#?}", self.chain[index])
    }
    pub fn print_chain_transactions(&self) {
        let mut vec_trans = Vec::new();
        for block in &self.chain {
            if block.transactions.len() > 1 {
                vec_trans.push(&block.transactions[1])
            }
        }
        println!("{:#?}", vec_trans)
    }
    ...
    pub fn generate_new_block(&mut self) -> bool {
        let header = Blockheader {
            timestamp: time::now().to_timespec().sec as f64,
            nonce: 0,
            merkle: String::new(),
